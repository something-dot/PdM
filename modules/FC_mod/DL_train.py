...
from preprocessing import flatten, curve_shift
...
def test_train_split_standardize(df, seed=456, data_split=0.2):
    ...
    # If scaler exists import, otherwise
    scaler = StandardScaler().fit(df_train_0_x)
    dump(scaler, "..\output\FC_output\DL_output\std_scaler.bin", compress=True)
...
if __name__ == '__main__':
    try:
        df = pd.read_csv(pd.read_csv(sys.argv[1]))  # "..\DE_output\df.csv"
    except IndexError:
        df = pd.read_csv("..\output\DE_output\df.csv")

    # Drop the columns, We don't want these features
    df = df.drop(['time', 'x28', 'x61'], axis=1)

    # Curve Shift data frame, delete code below if it is already curve shifted
    df = curve_shift(df, 10)

    # If no mode, just make a model
    if not path.exists('..\output\FC_output\\autoencoder.h5'):
        ...
        path = '..\output\FC_output\DL_output\\autoencoder.h5'
        best_model = hyper_param_tune(df, activation_list, optimizer_list, learn_rate_list, path=path)
        best_model.save('..\output\FC_output\DL_output\\autoencoder.h5')

        train_test_valid_standardized_data = test_train_split_standardize(df)

        roc_curve_plot(path, train_test_valid_standardized_data[3], train_test_valid_standardized_data[5])
        test_metrics_print(path, train_test_valid_standardized_data[3], train_test_valid_standardized_data[5])
    else:
        print("Autoencoder already exists")
        # path = '..\output\FC_output\DL_output\\autoencoder.h5'
        # scaler = load("..\output\FC_output\DL_output\std_scaler.bin")
        # df_rescaled = scaler.transform(df.drop(['y'], axis=1))
        #
        #
        # pred_y = get_predictions(path=path, rescaled=df_rescaled, original=df)
        # pd.DataFrame(pred_y).to_csv("..\output\FC_pred\pred.csv")
